<section>
    ...
    <form id="profile-form" method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6" enctype="multipart/form-data">
        ...
        <div class="flex flex-col items-center space-y-4">
            ...
            <p class="text-xs text-gray-500">Click the camera icon to upload a new photo (Max: 2MB)</p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <x-text-input id="nip" name="nip" type="text" maxlength="18" pattern="[0-9]*" inputmode="numeric" class="mt-1 block w-full" :value="old('nip', $user->nip)" placeholder="198402132009121001" />
                <x-input-error class="mt-2" :messages="$errors->get('nip')" />
            </div>
            <div>
                <x-input-label for="jabatan" :value="__('Jabatan')" />
                <x-text-input id="jabatan" name="jabatan" type="text" class="mt-1 block w-full" :value="old('jabatan', $user->jabatan)" placeholder="Jabatan" />
                <x-input-error class="mt-2" :messages="$errors->get('jabatan')" />
            </div>
            <div>
                <x-input-label for="role" :value="__('Role')" />
                <x-text-input id="role" name="role" type="text" class="mt-1 block w-full bg-gray-100" :value="$user->role === 'ppk' ? 'Pejabat Pembuat Komitmen' : $user->getRoleDisplayName()" readonly />
            </div>
            <div>
                <x-input-label for="phone" :value="__('Phone Number')" />
                <x-text-input id="phone" name="phone" type="tel" class="mt-1 block w-full" :value="old('phone', $user->phone)" placeholder="+62 812 3456 7890" />
                <x-input-error class="mt-2" :messages="$errors->get('phone')" />
            </div>
            <div>
                <x-input-label for="address" :value="__('Address')" />
                <textarea id="address" name="address" rows="3" class="mt-1 block w-full border-gray-300 rounded-md" placeholder="Enter your full address">{{ old('address', $user->address) }}</textarea>
                <x-input-error class="mt-2" :messages="$errors->get('address')" />
            </div>
            <div>
                <x-input-label for="pangkat" :value="__('Pangkat')" />
                <x-text-input id="pangkat" name="pangkat" type="text" class="mt-1 block w-full" :value="old('pangkat', $user->pangkat)" placeholder="Pangkat" />
                <x-input-error class="mt-2" :messages="$errors->get('pangkat')" />
            </div>
            <div>
                <x-input-label for="golongan" :value="__('Golongan')" />
                <x-text-input id="golongan" name="golongan" type="text" class="mt-1 block w-full" :value="old('golongan', $user->golongan)" placeholder="Golongan" />
                <x-input-error class="mt-2" :messages="$errors->get('golongan')" />
            </div>
            <div>
                <x-input-label for="unit_kerja" :value="__('Unit Kerja')" />
                <x-text-input id="unit_kerja" name="unit_kerja" type="text" class="mt-1 block w-full" :value="old('unit_kerja', $user->unit_kerja)" placeholder="Unit Kerja" />
        ...
        <div class="flex items-center gap-4">
            ...
            <div id="profile-error" class="hidden">
                <p class="text-sm text-red-600 dark:text-red-400">Error updating profile. Please try again.</p>
            </div>
        ...
        function previewImage(input) {
            if (input.files && input.files[0]) {
                ...
                reader.onload = function(e) {
                    ...
                    imgElement.src = e.target.result + '?v=' + new Date().getTime();
