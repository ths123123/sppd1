...
<div class="min-h-screen bg-gray-50">
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <?php endif; ?>
        <?php if(isset($errors) && $errors->has('rejection_reason')): ?>
            <script>
                window.addEventListener('DOMContentLoaded', function() {
                    showRejectModal('<?php echo e(old('reject_id') ?? ''); ?>', '<?php echo e(old('reject_kode') ?? ''); ?>');
                });
            </script>
        <?php endif; ?>
                    ...
                    <div class="flex items-center justify-between p-2 bg-gray-50 rounded-lg">
                        <div class="flex items-center space-x-3">
                            <div class="w-2 h-2 rounded-full
                                <?php if($activity->status === 'completed'): ?> bg-green-500
                                <?php elseif($activity->status === 'rejected'): ?> bg-red-500
                                <?php else: ?> bg-yellow-500 <?php endif; ?>">
                            </div>
                            <div>
                                <p class="text-sm font-medium text-gray-900">
                                    <?php echo e($activity->travelRequest->kode_sppd); ?> - <?php echo e($activity->travelRequest->user->name); ?>

                                </p>
                                <p class="text-xs text-gray-600">
                                    <?php echo e(ucfirst($activity->status)); ?> • <?php echo e($activity->created_at->format('d M H:i')); ?>

                                </p>
                            </div>
                        </div>
                        <span class="text-xs px-2 py-1 rounded-full font-medium
                            <?php if($activity->status === 'completed'): ?> bg-green-100 text-green-800
                            <?php elseif($activity->status === 'rejected'): ?> bg-red-100 text-red-800
        ...
        <div x-data="approvalTable" x-init="initTable()" class="bg-white shadow-md rounded-2xl overflow-hidden">
            ...
            <div id="approval-table-ajax">
                <?php echo $__env->make('approval.pimpinan.partials.approval_requests_table', ['requests' => $requests], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
            </div>
            ...
            <div id="rejectModal" 
                ...
                <div class="relative top-20 mx-auto p-8 max-w-md w-full bg-white rounded-xl shadow-xl transform transition-all" 
                    ...
                    <form id="rejectForm" onsubmit="event.preventDefault(); window.dispatchEvent(new CustomEvent('approval:submitReject'))">
                        ...
                        <input type="hidden" name="reject_id" value="<?php echo e(old('reject_id')); ?>">
                        <input type="hidden" name="reject_kode" value="<?php echo e(old('reject_kode')); ?>">
                        <div class="mb-6">
                            <div class="mx-auto flex items-center justify-center h-16 w-16 rounded-full bg-red-100 mb-4">
                                <svg class="h-10 w-10 text-red-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                            ...
                            <div class="mt-6">
                                ...
                                <textarea id="rejection_reason" name="rejection_reason"
                                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent transition-all <?php if(isset($errors) && $errors->has('rejection_reason')): ?> border-red-500 <?php endif; ?>"
                                        rows="3"
                                        placeholder="Jelaskan alasan penolakan SPPD ini..."
                                        required><?php echo e(old('rejection_reason')); ?></textarea>
                                <?php if(isset($errors) && $errors->has('rejection_reason')): ?>
                                    <div class="text-red-600 text-xs mt-1"><?php echo e($errors->first('rejection_reason')); ?></div>
                                <?php endif; ?>
                            </div>
                        </div>
                        <div class="flex justify-center gap-4 mt-6">
    ...
    <div class="w-full flex flex-col items-center justify-center pb-8 pt-8 text-center">
        <div class="text-gray-500 text-sm">
            Update terakhir<br><span class="font-semibold"><?php echo e(now()->format('d M Y H:i')); ?> WIB</span>
        </div>
    </div>
</div>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('approvalTable', () => ({
        ...
        filterTable(page = null) {
            ...
            console.log('[approvalTable] User role:', '<?php echo e(Auth::user()->role); ?>');
            
            // Debug info sebelum fetch
            console.log('[approvalTable] Fetching from URL:', '<?php echo e(route("approval.pimpinan.ajax")); ?>?' + params);
            
            fetch('<?php echo e(route("approval.pimpinan.ajax")); ?>?' + params, {
                headers: {
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\pkl\SPPD-KP1\SPPD-KPUKP1\resources\views/approval/pimpinan/approval_requests.blade.php ENDPATH**/ ?>